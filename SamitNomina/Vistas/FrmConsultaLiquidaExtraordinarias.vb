Imports SamitCtlNet
Public Class FrmConsultaLiquidaExtraordinarias
    ...
    Public Function ValidaCampos() As Boolean
        ...
        Else
            If txtOficina.ValordelControl = "" Or txtOficina.DescripciondelControl = "No Se Encontraron Registros" Or txtOficina.DescripciondelControl = "" Then
                ...
                txtOficina.Focus()
                Return False
            ElseIf txtNomina.ValordelControl = "" Or txtNomina.DescripciondelControl = "No Se Encontraron Registros" Or txtNomina.DescripciondelControl = "" Then
                HDevExpre.MensagedeError("El campo Nomina no puede estar vacío o contener valores invalidos!..")
                txtNomina.Focus()
                Return False
            Else : Return True
            End If
        ...
        If ValidaCampos() Then
            ...
            If rgTipoLiquida.SelectedIndex = 0 Then
                ...
                ElseIf txtContratos.ValordelControl <> "" And txtContratos.DescripciondelControl = "No Se Encontraron Registros" Or txtContratos.ValordelControl <> "" And txtContratos.DescripciondelControl = "" Then
                    ...
                    Exit Sub
                End If
                Dim NominaContrato As String = SMT_AbrirTabla(ObjetoApiNomina, "SELECT Nomina AS Codigo FROM Contratos Where IdContrato='" + txtContratos.ValordelControl + "'").Rows(0)(0).ToString
                FrmConceptosLiquidaExtraordinaria.P_Nomina = NominaContrato
                SecNomina = NominaContrato
            Else
                If txtOficina.ValordelControl <> "" And txtOficina.DescripciondelControl <> "No Se Encontraron Registros" And txtOficina.DescripciondelControl <> "" Or txtNomina.ValordelControl <> "" And txtNomina.DescripciondelControl <> "No Se Encontraron Registros" And txtNomina.DescripciondelControl <> "" Or txtDependencia.ValordelControl <> "" And txtDependencia.DescripciondelControl <> "No Se Encontraron Registros" And txtDependencia.DescripciondelControl <> "" Or txtCargos.ValordelControl <> "" And txtCargos.DescripciondelControl <> "No Se Encontraron Registros" And txtCargos.DescripciondelControl <> "" Then
                    sql = " C.Terminado=0 And C.Nomina=" + txtNomina.ValordelControl
                    ...
                    End If
                End If
                FrmConceptosLiquidaExtraordinaria.P_Nomina = txtNomina.ValordelControl
                SecNomina = txtNomina.ValordelControl
            End If
            FrmConceptosLiquidaExtraordinaria.Fomulario = Formu
            FrmConceptosLiquidaExtraordinaria.ShowDialog()
            If Continua Then
                FrmLiquidacionExtraordinaria.P_Fecha = dteFecha.DateTime
                FrmLiquidacionExtraordinaria.P_Nomina = SecNomina
                FrmLiquidacionExtraordinaria.P_Oficina = txtOficina.ValordelControl
                FrmLiquidacionExtraordinaria.ConsultarLiquidacion("", sql)
                LimpiarCampos()
                MyBase.Close()
            End If
        End If
